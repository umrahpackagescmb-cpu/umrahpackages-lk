...
function ToggleCell({
  ...
  return (
    <div className="flex flex-col gap-1">
      <Switch checked={checked} onCheckedChange={handleChange} disabled={pending} aria-label={label} />
...
export function PackageTable({ packages: initialPackages }: { packages: Package[] }) {
  ...
  if (packages.length === 0) {
    return <EmptyState title="No packages found" description="Packages created by agencies will appear here." />;
  }
            ...
            <TableRow key={pkg.id}>
              <TableCell>
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-muted-foreground">{pkg.agency.name}</p>
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell>
                <ToggleCell
                  ...
                  onToggle={async (next) => {
                    const result = await setPackageFeatured(pkg.id, next);
                    if (result.ok) updatePackage(pkg.id, { isFeatured: next });
                    return result;
                  }}
                />
              </TableCell>
              <TableCell>
                <ToggleCell
                  ...
                  onToggle={async (next) => {
                    ...
                    if (result.ok) updatePackage(pkg.id, { isPublished: next });
                    return result;
                  }}
                />
              </TableCell>
